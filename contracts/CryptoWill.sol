...
contract CryptoWill {
    ...
    function transferOwnership(address newOwner) public onlyOwner { // called by owner to change the ownership
        owner = newOwner;
        beneficiary = owner;
    }

    //called by owner to add/modify an heir; inheritance shares are directly propotional to the points assigned
    function setHeir(address heir, uint8 inheritancePoints, uint periodInDays) public returns (address) {
    ...
    function claimInheritance() public OnlyHeir {
        require (beneficiary == owner, "Account is locked"); // account is locked
        require (now > lastCheckInTime + checkInPeriod, "owner was active recently");  // owner was active recently
    ...
    function getBeneficiary() public returns (address) { return beneficiary; }
